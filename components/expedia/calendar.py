...
class Calendar(BaseComponent):
    ...
    def wait_until_loaded(self):
        ...
        self.get_hijos(self.__SELECTED_DATES_LOCATOR)

    def get_start_date(self) -> str:
        """Get selected start date."""
        dates = self.get_hijos(self.__SELECTED_DATES_LOCATOR)
        if len(dates) == 2:
            return dates[0].text
        else:
            return None
    ...
    def get_first_month(self) -> str:
        ...
        months = self.get_hijos(self.__MONTHS_XPATH)
        if len(months) == 2:
    ...
    def apply(self):
        """Apply changes."""
        apply = self.get_hijo(self.__APPLY_XPATH)
        apply.click()
    ...
    def select_dates(self, first_day: int, second_day: int):
        ...
        first_day_element = self.get_hijo(first_day_xpath)
        first_day_element.click()
        second_day_xpath = f"//*[@data-day='{second_day}' and not(contains(@class, 'is-disabled'))]"
        second_day_element = self.get_hijo(second_day_xpath)
